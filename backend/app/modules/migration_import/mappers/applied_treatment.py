        ...
        if twin_uuid is not None:
            ...
            if twin_item_id is not None and twin_treatment_id is not None:
                await ctx.resolver.set(
                    ...
                    ebyzomdental_table="planned_treatment_items",
                    ebyzomdental_id=twin_item_id,
                )
                await ctx.resolver.set(
                    entity_type="applied_treatment_record",
                    canonical_uuid=canonical_uuid,
                    source_system=source_system,
                    ebyzomdental_table="treatments",
                    ebyzomdental_id=twin_treatment_id,
                )
            else:
        ...
        if is_non_clinical or (has_no_catalog_variant and has_no_clinical_type):
            ...
            if note_id is not None:
                ...
                await ctx.resolver.set(
                    ...
                    canonical_uuid=canonical_uuid,
                    source_system=source_system,
                    ebyzomdental_table="clinical_notes",
                    ebyzomdental_id=note_id,
                )
            else:
