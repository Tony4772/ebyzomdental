...
This module lives inside ``migration_import`` because EBYZOM dental is a
...
1. **Clinical type** — EBYZOM dental ``TreatmentType`` enum (or ``None``
...
# ---------------------------------------------------------------------------
# IdTipoODG → EBYZOM dental TreatmentType (clinical_type) string.
...
# ---------------------------------------------------------------------------
# IdTipoODG → EBYZOM dental treatment_scope. Drives how the destination catalog
...
def clinical_type_for_tipo_odg(tipo_odg: int | None) -> str | None:
    """Return the EBYZOM dental ``TreatmentType`` value for a Gesdén
