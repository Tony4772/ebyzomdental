...
than EBYZOM dental's flat Payment model — allocations land in `RawEntity`
...
# Gesdén ``DCobros.Tipo`` numeric code → EBYZOM dental payment method
...
class PaymentMapper:
    ...
    async def apply(
        ...
        *,
        ...
        await ctx.resolver.set(
            ...
            canonical_uuid=canonical_uuid,
            source_system=source_system,
            ebyzomdental_table="payments",
            ebyzomdental_id=first_payment_id,
        )
    ...
    async def _resolve_refund_target(
        ...
        in EBYZOM dental; the caller emits
    ...
    async def _apply_refund(
        ...
        *,
        ...
        EBYZOM dental ``Payment``. Target resolution lives in
        ...
        await ctx.resolver.set(
            ...
            canonical_uuid=canonical_uuid,
            source_system=source_system,
            ebyzomdental_table="refunds",
            ebyzomdental_id=refund.id,
        )
        return refund.id
