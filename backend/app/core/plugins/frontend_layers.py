...
DEFAULT_FRONTEND_ROOT = Path(settings.EBYZOMDENTAL_FRONTEND_ROOT)
...
def _translate_for_frontend(path: Path) -> str:
    ...
    The backend sees layers at ``{EBYZOMDENTAL_MODULE_PKG_ROOT}/<name>/...``;
    the frontend container mounts the same filesystem at
    ``{EBYZOMDENTAL_MODULE_LAYERS_MOUNT}``. When the two paths differ we
    ...
    """
    pkg_root = Path(settings.EBYZOMDENTAL_MODULE_PKG_ROOT)
    mount = Path(settings.EBYZOMDENTAL_MODULE_LAYERS_MOUNT)
    if pkg_root == mount:
        return str(path)
    try:
