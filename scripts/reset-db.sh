...
docker compose exec -T db psql -U ebyzomdental -d ebyzomdental_clinic -c "DELETE FROM alembic_version;" 2>/dev/null || true

# Drop all tables (in correct order to handle foreign keys)
docker compose exec -T db psql -U ebyzomdental -d ebyzomdental_clinic << 'EOF'
DO $$
DECLARE
    r RECORD;
BEGIN
...
for i in {1..30}; do
  count=$(docker compose exec -T db psql -U ebyzomdental -d ebyzomdental_clinic -tA -c \
