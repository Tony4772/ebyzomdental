...
# Usage: /usr/local/bin/ebyzomdental-reset-demo.sh [es|en]
#
# This script lives on the demo host at /usr/local/bin/ebyzomdental-reset-demo.sh
# and is invoked by /etc/cron.d/ebyzomdental-reset-demo every night at 04:00.
# This file is the source of truth — see scripts/ebyzomdental-reset-demo.cron
# for the matching crontab. Deploy with:
#
#   scp scripts/ebyzomdental-reset-demo.sh     root@<host>:/usr/local/bin/
#   scp scripts/ebyzomdental-reset-demo.cron   root@<host>:/etc/cron.d/ebyzomdental-reset-demo
#   ssh root@<host> 'chmod +x /usr/local/bin/ebyzomdental-reset-demo.sh \
#                  && chmod 644 /etc/cron.d/ebyzomdental-reset-demo'
set -euo pipefail

COOLIFY_PROJECT="wz49q8rmlqkhh9qun1kwgge8"
LANG_ARG="${1:-es}"
LOG_TAG="ebyzomdental-reset"
