...
Working notes for AI agents on EBYZOM dental.

## Project

EBYZOM dental — open-source dental clinic management software with a modular plugin architecture.
...
EBYZOM dental is built as independent modules under `backend/app/modules/<name>/` with matching Nuxt layers. Treat the boundary as a contract.
...
```bash
DATABASE_URL=postgresql+asyncpg://ebyzomdental:ebyzomdental_dev@db:5432/ebyzomdental_clinic
