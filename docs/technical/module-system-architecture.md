...
- Discovery principal: `importlib.metadata.entry_points(group="ebyzomdental.modules")`.
...
- Comunidad publica su módulo como paquete en PyPI con su propio `pyproject.toml` declarando entry point `ebyzomdental.modules`.
...
- Meta-paquete `ebyzomdental-standard` agrupa core + módulos oficiales para instalación single-command.
...
- CLI `ebyzomdental modules install <name>` hace:
...
- Entry point `ebyzomdental-admin` (console script) se añade en fase 2 cuando exista distribución CLI real.
...
```
ebyzomdental-billing/                         # repo community o directorio interno
├── pyproject.toml                         # declara entry point "ebyzomdental.modules"
├── ebyzomdental_billing/
...
```python
# ebyzomdental_billing/manifest.py
MANIFEST = {
    # Identidad
    "name": "billing",
    "version": "1.0.0",
...
```python
# ebyzomdental_billing/__init__.py
from ebyzomdental.core.plugins import BaseModule
...
```toml
[project.entry-points."ebyzomdental.modules"]
billing = "ebyzomdental_billing:BillingModule"
```
...
1. **Entry points Python** (principal): `importlib.metadata.entry_points(group="ebyzomdental.modules")`. Cubre módulos oficiales e instalados desde PyPI.
2. **Filesystem scan** (modo dev): escaneo de `backend/app/modules/` buscando paquetes con `manifest.py`. Se incluye en discover si no hay entry point equivalente. Controlado por env var `EBYZOMDENTAL_DEV_MODULE_SCAN=true`.
...
```yaml
# ebyzomdental_billing/data/default_tax_rates.yaml
...
Flujo `ebyzomdental modules install <name>`:
...
```ts
// ebyzomdental_billing/frontend/slots.ts
...
15. **Install community module simulado**: crear paquete de prueba `ebyzomdental-foo` en fixture, `pip install -e`, `cli modules install foo`, reinicio, verificar router montado, migración aplicada, nav item presente.
...
- **CLI como `ebyzomdental-admin` console script global**: fuera, `python -m app.cli` suficiente.
...
- Cualquier desarrollador externo puede crear `ebyzomdental-mi-modulo`, publicarlo en PyPI, y una clínica self-hosted lo instala con un comando.
