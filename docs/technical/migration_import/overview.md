...
file produced by `dental-bridge` into the current EBYZOM Dental clinic.
...
| `migration_import_entity_mappings` | Idempotency keystone: `(clinic_id, source_system, entity_type, canonical_uuid)` → EBYZOM Dental row. |
...
2. Resolves cross-entity references via the resolver (never trusts
   raw canonical UUIDs as EBYZOM Dental FKs).
