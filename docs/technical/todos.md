...
### What is EBYZOM Dental

EBYZOM Dental is open source dental clinic management software designed to be:
...
EBYZOM Dental is designed so the **entire dental industry** can build modules.
...
**Goal:** EBYZOM Dental is the industry standard.
...
| Feature | Description |
|---------|-------------|
| Mapping tool | Map source fields → EBYZOM Dental |
...
- [ ] Distributed rate-limit store — replace in-process counters once EBYZOM Dental runs with multiple backend workers
...
**Primary metric:** Active clinics using EBYZOM Dental daily.
